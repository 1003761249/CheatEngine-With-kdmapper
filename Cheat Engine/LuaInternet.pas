...
begin
  if lua_gettop(l)>0 then
    luaclass_newClass(L, TWinInternet.create({$ifdef altname}'Deemo2 Engine'{$else}cename{$endif}+' : luascript-'+Lua_ToString(L,1)))
  else
    luaclass_newClass(L, TWinInternet.create({$ifdef altname}'Deemo2 Engine'{$else}cename{$endif}+' : luascript'));
