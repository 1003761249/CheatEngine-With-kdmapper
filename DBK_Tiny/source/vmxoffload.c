...
*/
{
	...
	DbgPrintEx(0,0,"First time run. Initializing vmm section");
	//__debugbreak();
	...
	if (vmm)
	{
		...
		DbgPrintEx(0,0,"Allocated memory at virtual address %p (physical address %I64x)\n", vmm, MmGetPhysicalAddress(vmm));
		vmmPA = MmGetMdlPfnArray(DBVMMDL)[0] << 12;
		DbgPrintEx(0,0,"(physical address %I64x)\n", vmmPA);
		...
		if (OpenedFile == STATUS_SUCCESS)
		{
			...
			{
				DbgPrintEx(0,0,"File bigger than 4MB. Big retard detected\n");
				return;
			}
			...
			if (statusblock.Status == STATUS_SUCCESS)
			{
				...
				DbgPrintEx(0,0,"The startsector=%d (that's offset %d)\n", startsector, startsector * 512);
				...
				if (statusblock.Status == STATUS_SUCCESS)
				{
					...
					DbgPrintEx(0,0,"Setting up initial paging table for vmm\n");
					...
					*(PUINT64)(&PageDir[1]) = 0x00200000; //00200000-00400000
					...
					DbgPrintEx(0,0,"&NewGDTDescriptor=%p, &NewGDTDescriptor.limit=%p, &NewGDTDescriptor.base=%p\n", &NewGDTDescriptor, &NewGDTDescriptor.limit, &NewGDTDescriptor.base);
					DbgPrintEx(0,0,"NewGDTDescriptor.limit=%x\n", NewGDTDescriptor.limit);
					DbgPrintEx(0,0,"NewGDTDescriptor.base=%p\n", NewGDTDescriptor.base);

					NewGDTDescriptorVA = (UINT_PTR)&NewGDTDescriptor;


					maxPA.QuadPart = 0x003fffffULL; //allocate 4k at the lower 4MB
					DbgPrintEx(0,0,"Before enterVMM2 alloc: maxPA=%I64x\n", maxPA.QuadPart);
					...
					if (enterVMM2)
					{
						...
						DbgPrintEx(0,0,"enterVMM is located at %p (%I64x)\n", enterVMM, MmGetPhysicalAddress(enterVMM).QuadPart);
						DbgPrintEx(0,0,"enterVMM2 is located at %p (%I64x)\n", enterVMM2, MmGetPhysicalAddress(enterVMM2).QuadPart);


						DbgPrintEx(0,0,"Copying function till end\n");
						//copy memory
						...
						DbgPrintEx(0,0,"size is %d", i);

						RtlCopyMemory(enterVMM2, original, i);
						DbgPrintEx(0,0,"Copy done\n");
					}
					else
					{
						DbgPrintEx(0,0,"Failure allocating enterVMM2\n");
						return;
					}
					...
					//allocate 4 pages
					DbgPrintEx(0,0,"Allocating memory for the temp pagedir\n");
					TemporaryPagingSetup = ExAllocatePool(PagedPool, 4 * 4096);
					if (TemporaryPagingSetup == NULL)
					{
						DbgPrintEx(0,0,"TemporaryPagingSetup==NULL!!!\n");
						return;
					}
					...
					DbgPrintEx(0,0,"TemporaryPagingSetupPA = (%I64x) (Should be %I64x)\n", (UINT64)TemporaryPagingSetupPA, (UINT64)MmGetPhysicalAddress(TemporaryPagingSetup).QuadPart);
#ifdef AMD64			
					DbgPrintEx(0,0,"Setting up temporary paging setup for x64\n");
...
#else
					DbgPrintEx(0,0,"Setting up temporary paging setup\n");
					...
					DbgPrintEx(0,0,"Temp paging has been setup\n");
					...
					DbgPrintEx(0,0,"enterVMM2PA=%llx\n", enterVMM2PA);
					DbgPrintEx(0,0,"originalstatePA=%llx\n", originalstatePA);
					...
					if (contiguousMDL) {
						...
						DbgPrintEx(0,0,"contiguous PA =%llx\n", initvars->contiguousmemory);
						initvars->contiguousmemorysize = 8;
						ExFreePool(contiguousMDL);
					}
					else
						DbgPrintEx(0,0,"Failed allocating 32KB of contiguous memory");
			...
			DbgPrintEx(0,0,"Opened and processed: %S\n", filename.Buffer);
		}
		else
		{
			DbgPrintEx(0,0,"Failure opening the file. Status=%x  (filename=%S)\n", OpenedFile, filename.Buffer);
		}
...
void vmxoffload(void)
{
	...
	DbgPrintEx(0,0,"vmxoffload\n");



	DbgPrintEx(0,0,"initializedvmm=%d\n", initializedvmm); 
	if (initializedvmm)
	{
		DbgPrintEx(0,0,"cpunr=%d\n",cpunr());
		
		DbgPrintEx(0,0,"Storing original state\n");
		originalstate->cpucount=getCpuCount();
		DbgPrintEx(0,0,"originalstate->cpucount=%d",originalstate->cpucount);
		...
		DbgPrintEx(0,0,"originalstate->originalLME=%d",originalstate->originalLME);

		
		originalstate->cr0=(UINT_PTR)getCR0();
		

		DbgPrintEx(0,0,"originalstate->cr0=%I64x",originalstate->cr0);
		...
		/*
		{
			...
			for (xxx=0; xxx<8; xxx++)
			{
				DbgPrintEx(0,0,"%x ",x[xxx]);
			}
		}
		*/

		originalstate->cr2=(UINT_PTR)getCR2();
		DbgPrintEx(0,0,"originalstate->cr2=%I64x",originalstate->cr2);
		/*
		{
			int xxx;
			unsigned char *x;
		...
		//DbgPrintEx(0,0,"originalstate->cr3=%I64x",originalstate->cr3);

		originalstate->cr4=(UINT_PTR)getCR4();
		//DbgPrintEx(0,0,"originalstate->cr4=%I64x",originalstate->cr4);
		...
		//DbgPrintEx(0,0,"originalstate->ss=%I64x",originalstate->ss);
		...
		//DbgPrintEx(0,0,"originalstate->cs=%I64x",originalstate->cs);
		...
		//DbgPrintEx(0,0,"originalstate->ds=%I64x",originalstate->ds);
		...
		//DbgPrintEx(0,0,"originalstate->es=%I64x",originalstate->es);
		...
		//DbgPrintEx(0,0,"originalstate->fs=%I64x",originalstate->fs);
		...
		//DbgPrintEx(0,0,"originalstate->gs=%I64x",originalstate->gs);
		originalstate->ldt=GetLDT();
		//DbgPrintEx(0,0,"originalstate->ldt=%I64x",originalstate->ldt);
		originalstate->tr=GetTR();
		//DbgPrintEx(0,0,"originalstate->tr=%I64x",originalstate->tr);		


		originalstate->fsbase=readMSR(0xc0000100);
		originalstate->gsbase=readMSR(0xc0000101);

		//DbgPrintEx(0,0,"originalstate->fsbase=%I64x originalstate->gsbase=%I64x\n", originalstate->fsbase, originalstate->gsbase);
		...
		//DbgPrintEx(0,0,"originalstate->gdtbase=%I64x",originalstate->gdtbase);
		//DbgPrintEx(0,0,"originalstate->gdtlimit=%I64x",originalstate->gdtlimit);

		GetIDT(&idt);
		originalstate->idtbase=(ULONG_PTR)idt.vector;
		originalstate->idtlimit=idt.wLimit;

		//DbgPrintEx(0,0,"originalstate->idtbase=%I64x",originalstate->idtbase);
		//DbgPrintEx(0,0,"originalstate->idtlimit=%I64x",originalstate->idtlimit);
		...
		//DbgPrintEx(0,0,"originalstate->rsp=%I64x",originalstate->rsp);
		originalstate->rbp=getRBP();
		//DbgPrintEx(0,0,"originalstate->rbp=%I64x",originalstate->rbp);

		originalstate->rax=getRAX();
		//DbgPrintEx(0,0,"originalstate->rax=%I64x",originalstate->rax);
		originalstate->rbx=getRBX();
		//DbgPrintEx(0,0,"originalstate->rbx=%I64x",originalstate->rbx);
		originalstate->rcx=getRCX();
		//DbgPrintEx(0,0,"originalstate->rcx=%I64x",originalstate->rcx);
		originalstate->rdx=getRDX();
		//DbgPrintEx(0,0,"originalstate->rdx=%I64x",originalstate->rdx);
		originalstate->rsi=getRSI();
		//DbgPrintEx(0,0,"originalstate->rsi=%I64x",originalstate->rsi);
		originalstate->rdi=getRDI();
		//DbgPrintEx(0,0,"originalstate->rdi=%I64x",originalstate->rdi);
#ifdef AMD64
		originalstate->r8=getR8();
		//DbgPrintEx(0,0,"originalstate->r8=%I64x",originalstate->r8);
		originalstate->r9=getR9();
		//DbgPrintEx(0,0,"originalstate->r9=%I64x",originalstate->r9);
		originalstate->r10=getR10();
		//DbgPrintEx(0,0,"originalstate->r10=%I64x",originalstate->r10);
		originalstate->r11=getR11();
		//DbgPrintEx(0,0,"originalstate->r11=%I64x",originalstate->r11);
		originalstate->r12=getR12();
		//DbgPrintEx(0,0,"originalstate->r12=%I64x",originalstate->r12);
		originalstate->r13=getR13();
		//DbgPrintEx(0,0,"originalstate->r13=%I64x",originalstate->r13);
		originalstate->r14=getR14();
		//DbgPrintEx(0,0,"originalstate->r14=%I64x",originalstate->r14);
		originalstate->r15=getR15();
		//DbgPrintEx(0,0,"originalstate->r15=%I64x",originalstate->r15);
#endif
		...
		//DbgPrintEx(0,0,"originalstate->rip=%llx",originalstate->rip);

		//DbgPrintEx(0,0,"Calling entervmm2. (Originalstate=%p (%llx))\n",originalstate,originalstatePA);
		...
		//DbgPrintEx(0,0,"Returned from enterVMMPrologue\n");
...
void vmxoffload_override(CCHAR cpunr, PKDEFERRED_ROUTINE Dpc, PVOID DeferredContext, PVOID *SystemArgument1, PVOID *SystemArgument2)
{
	...
	PHYSICAL_ADDRESS LowAddress, HighAddress, SkipBytes;
	PMDL mdl;
	DbgPrintEx(0,0,"vmxoffload_override\n");
	LowAddress.QuadPart = 0;
	...
	if (mdl)
	{
		...
		DbgPrintEx(0,0,"vmxoffload_override: mi=%p\n", mi);
		
		mi->List = ExAllocatePool(NonPagedPool, sizeof(UINT64) * 16);

		DbgPrintEx(0,0,"vmxoffload_override: mi->list=%p\n", mi->List);
...
{
	int c = cpunr();
	DbgPrintEx(0,0,"vmxoffload_dpc: CPU %d\n", c);
	...
	if (SystemArgument1)
	{
		...
		DbgPrintEx(0,0,"mi->List=%p mi->Count=%d\n", mi->List, mi->Count);

		x=vmx_add_memory(mi->List, mi->Count);
		DbgPrintEx(0,0,"vmx_add_memory returned %x\n", x);
		...
		DbgPrintEx(0,0,"Error: SystemArgument1=NULL\n");
